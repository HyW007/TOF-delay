...
def generate_fluence(model_path, model_type, transformation_type, d, output_filename=None, tstart=None, tend=None, snmodel_dict={}):
    ...
    energy   = np.arange(15,50, 0.05) << u.MeV
    #energy = np.where(energy > 20 * u.MeV, 0 * u.MeV, energy)
...
def generate_fluence_tof(model_path, model_type, transformation_type, d, output_filename=None, tstart=None, tend=None, snmodel_dict={}):
    """Generate fluence files in SNOwGLoBES format.
    ...
    #energy with 0.2 MeV binning
    energy   = np.arange(1, 101, 0.2) << u.MeV
    #energy = np.where(energy > 10 * u.MeV, 0 * u.MeV, energy)
    #energy bins similar to SNOwGLoBES
    energy_t = (np.linspace(0, 100, 201)+0.25) << u.MeV
    flux = snmodel.get_flux_tof(t=snmodel.get_time(), E=energy,  distance=d, flavor_xform=flavor_transformation)
    fluence = flux.integrate('time', limits = times).integrate('energy', limits = energy_t)
    times = fluence.time
    #store the energy bin centers instead of the edges
    if output_filename is not None:
